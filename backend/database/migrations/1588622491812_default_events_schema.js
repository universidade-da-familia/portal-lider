  ...
  up() {
    this.create('default_events', table => {
      ...
      table
        ...
        .onUpdate('CASCADE')
        .onDelete('SET NULL');
      table.integer('organizator_hierarchy_id').unsigned().notNullable();
      table.integer('assistant_hierarchy_id').unsigned().notNullable();
      table.integer('assistant_will_become_id').unsigned();
      table.integer('assistant_current_event_id').unsigned();
      table.integer('participant_hierarchy_id').unsigned().notNullable();
      table.integer('participant_will_become_id').unsigned().notNullable();
