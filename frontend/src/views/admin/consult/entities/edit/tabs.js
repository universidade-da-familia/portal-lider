...
import ChurchsTable from '~/views/profile/churchsTable';
...
export default function EntityTabs({ match }) {
  ...
  const [addresses, setAddresses] = useState([
    {
      id: null,
      entity_id: parseInt(match.params.id, 10),
      type: '',
      other_type_name: '',
      cep: '',
      country: 'Brasil',
  ...
  function toggle(tab) {
    if (activeTab !== tab) {
      setActiveTab(tab);
      localStorage.setItem('@dashboard/adminProfileActiveTab', tab);
    }
  }
  ...
  function handleUpdateMinisteries(values) {
    ...
    values.entity_id = parseInt(match.params.id, 10);
  ...
  useEffect(() => {
    if (churchs && churchs.length > 0) {
      // setModalChurchData(churchs);
    }
  }, [churchs]);
  ...
  useEffect(() => {
    const storageTab = localStorage.getItem('@dashboard/adminProfileActiveTab');

    if (storageTab) {
      setActiveTab(storageTab);
    }

    dispatch(EntityActions.entityRequest(match.params.id));

    return () => {
      setSelectedChurch(null);

      store.getState().cep.data = {};
      store.getState().entity.data = {
        orders: [],
      };
    };
  }, []);
